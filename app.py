...
app.secret_key = 'daniel' # app.config['JWT_SECRET_KEY']
api = Api(app)
...
def expired_token_callback(_decrypted_header, decrypted_body):
    return jsonify({
...
def needs_fresh_token_callback(_decrypted_header, decrypted_body):
    return jsonify({
